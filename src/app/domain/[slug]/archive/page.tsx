  ...
  <ThemeToggle />
      ...
      <div className="ms-layout">
        ...
        <main style={{ padding: "24px" }}>
          <div style={{ marginBottom: "24px", borderBottom: "0.5px solid var(--ms-border)", paddingBottom: "16px", display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            <div>
              <p style={{ fontSize: "10px", color: "var(--ms-accent)", letterSpacing: "0.1em", textTransform: "uppercase", margin: "0 0 6px" }}>{domain.label} Archive</p>
              <h1 style={{ fontSize: "18px", fontWeight: 500, color: "var(--ms-text-strong)", margin: "0 0 6px" }}>{domain.desc}</h1>
              <p style={{ fontSize: "12px", color: "var(--ms-text-label)", margin: 0 }}>全{count ?? 0}件</p>
            </div>
            <Link href={"/domain/" + slug} style={{ fontSize: "11px", color: "var(--ms-accent)", textDecoration: "none", border: "0.5px solid var(--ms-accent)", padding: "6px 14px", borderRadius: "20px" }}>
              最新記事へ
            </Link>
          </div>
            ...
            <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: "10px" }}>
              {items.map((article) => (
                <li key={article.id} style={{ background: "var(--ms-bg-card)", border: "0.5px solid var(--ms-border)", borderLeft: "2px solid var(--ms-accent)", borderRadius: "0 8px 8px 0", padding: "16px" }}>
                  ...
                  <h2 style={{ fontSize: "14px", fontWeight: 500, margin: "0 0 4px", lineHeight: 1.5 }}>
                    <Link href={"/article/" + article.id} style={{ color: "var(--ms-text-primary)", textDecoration: "none" }}>
                      {article.title_ja ?? article.title}
                    </Link>
                  </h2>
                  {article.title_ja && (
                    <p style={{ fontSize: "11px", color: "var(--ms-text-label)", margin: "0 0 8px", lineHeight: 1.5 }}>{article.title}</p>
                  )}
