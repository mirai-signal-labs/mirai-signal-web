    ...
    <div style={{ background: "#080810", minHeight: "100vh" }}>
      ...
      <div className="ms-layout">
        ...
        <main style={{ padding: "24px" }}>
          <div style={{ marginBottom: "24px", borderBottom: "0.5px solid #1e1e30", paddingBottom: "16px", display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            <div>
              <p style={{ fontSize: "10px", color: "#534ab7", letterSpacing: "0.1em", textTransform: "uppercase", margin: "0 0 6px" }}>{domain.label} Archive</p>
              <h1 style={{ fontSize: "18px", fontWeight: 500, color: "#e8e6ff", margin: "0 0 6px" }}>{domain.desc}</h1>
              <p style={{ fontSize: "12px", color: "#444441", margin: 0 }}>全{count ?? 0}件</p>
            </div>
            <Link href={"/domain/" + slug} style={{ fontSize: "11px", color: "#534ab7", textDecoration: "none", border: "0.5px solid #534ab7", padding: "6px 14px", borderRadius: "20px" }}>
              最新記事へ
            </Link>
          </div>
            ...
            <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: "10px" }}>
              {items.map((article) => (
                <li key={article.id} style={{ background: "#0e0e1a", border: "0.5px solid #1e1e30", borderLeft: "2px solid #534ab7", borderRadius: "0 8px 8px 0", padding: "16px" }}>
                  ...
                  {article.title_ja && (
                    <p style={{ fontSize: "11px", color: "#444441", margin: "0 0 8px", lineHeight: 1.5 }}>{article.title}</p>
                  )}
